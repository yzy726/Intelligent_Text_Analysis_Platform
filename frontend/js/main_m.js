...
/**
 * 质构文析台 - 移动端主逻辑脚本
 * 核心逻辑与 PC 端 (main.js) 基本一致，主要区别在于：
 * 1. 增加了设备类型检测，自动重定向。
 * 2. 针对移动端交互习惯可能做出的微调 (如触摸事件、弹窗提示等)。
 */

// ==================== 设备检测与重定向 ====================
// 检测当前 UserAgent 是否为非移动端设备，如果是则自动跳转到 PC 版页面
if (!/Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent)) {
    window.location.href = 'zgwxt.html' + window.location.search;
}

// ==================== 全局状态变量 ====================
let fieldList = []; // 存储用户配置的提取字段列表
let currentFile = null; // 当前用户上传的文件对象
let currentFileUrl = ''; // 当前文件的本地预览 URL
let fileType = ''; // 当前文件类型标识：'pdf' 或 'image'
let pdfDoc = null; // PDF.js 解析后的 PDF 文档实例
let extractResult = []; // 存储最终的提取结果列表
let currentPage = 1; // 当前预览的 PDF 页码
let ocrText = ''; // 纯文本模式下拼接的 OCR 识别文本
let cachedOcrData = null; // 缓存后端返回的原始 OCR 结果
let currentProcessMode = 'ocr_only'; // 当前选择的处理模式
let currentDisplayMode = 'extract_only'; // 结果展示模式
let cachedOcrModel = ''; // 记录生成当前缓存 OCR 数据时使用的模型名称
let cachedPdfBlob = null; // 缓存后端生成的双层 PDF Blob 对象
...
window.onload = function() {
    initTabs(); // 初始化标签页切换逻辑
    loadFieldConfig(); // 从本地存储加载上次保存的字段配置
};

/**
 * 初始化顶部标签页 (Tabs) 的点击切换逻辑 (移动端)
 */
function initTabs() {
    ...
    tabBtns.forEach(btn => {
        // 移动端推荐使用 touchstart 或 click，这里保持 click 兼容性
...
function toggleExtractOptions() {
    ...
    if (isExtract) {
        ...
        void optionsArea.offsetWidth; // 触发重绘
...
/**
 * 调用后端 API，使用大语言模型 (LLM) 解析用户输入的自然语言需求
 */
...
async function confirmAndProcess() {
    ...
    try {
        ...
        if (!cachedOcrData || cachedOcrModel !== selectedModel) {
            ...
            // 移动端通常不显示详细的日志轮询，以节省性能和空间，这里保留代码但默认隐藏了 logArea
            if (logArea && !logArea.classList.contains('hidden')) {
                logInterval = setInterval(async () => {
                    try {
                        const logRes = await fetch(`${baseUrl}/ocr_logs?task_id=${taskId}`);
                        if (logRes.ok) {
                            const logData = await logRes.json();
                            if (logData.logs && logData.logs.length > 0) {
                                logArea.innerHTML = logData.logs.join('<br>');
